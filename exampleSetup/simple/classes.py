...
class DefaultAllocator(AllocatorBase):
    ...
    Motor layout (NED body frame, X-forward, Y-right):
    ...
    Forward mixer:
      T  = kf*(ω0² + ω1² + ω2² + ω3²)
      Mx = l*kf*(ω3² - ω1²)  # roll from left/right differential
      My = l*kf*(ω0² - ω2²)  # pitch from front/back differential
      Mz = km*(ω0² - ω1² + ω2² - ω3²)  # yaw from CW/CCW differential

    Inverted mixer:
      ω0² = T/(4kf) + My/(2*l*kf) + Mz/(4*km)
      ω1² = T/(4kf) - Mx/(2*l*kf) - Mz/(4*km)
      ω2² = T/(4kf) - My/(2*l*kf) + Mz/(4*km)
      ω3² = T/(4kf) + Mx/(2*l*kf) - Mz/(4*km)
    """
    ...
    def allocate(
        ...
        thrust_per_motor[0] = T / (4 * kf) + My / (2 * l * kf) + Mz / (4 * km)
        thrust_per_motor[1] = T / (4 * kf) - Mx / (2 * l * kf) - Mz / (4 * km)
        thrust_per_motor[2] = T / (4 * kf) - My / (2 * l * kf) + Mz / (4 * km)
        thrust_per_motor[3] = T / (4 * kf) + Mx / (2 * l * kf) - Mz / (4 * km)
...
class CirclePilot(PilotBase):
    """
    Simple altitude-hold pilot with optional circular trajectory.

    For altitude-only mode: set radius=0, angular_speed=0, and provide target altitude.
    Uses cascaded PD: altitude error → desired vertical velocity → desired thrust.
    """
    ...
    def __init__(
        ...
        # Gains for altitude hold
        self.Kp_alt = 2.0    # altitude → desired vertical velocity
        self.Kd_alt = 1.0    # vertical velocity error → acceleration
        self.Kp_att = 5.0    # attitude
        self.Kd_att = 2.0    # attitude damping
    ...
    def compute_control(
        ...
        # State
        p = state.position.vec.ravel().astype(np.float64)
        R = _get_body_to_inertial(state.quaternion.as_np()).astype(np.float64)
        v = (R @ state.velocity.vec).ravel().astype(np.float64)
        omega = state.omega.vec.ravel().astype(np.float64)
        euler = state.quaternion.to_euler()

        # Target altitude (NED: -altitude = up)
        target_z = -self.altitude

        # Altitude error
        alt_err = target_z - p[2]
        des_vz = self.Kp_alt * alt_err

        # Vertical velocity error → desired acceleration
        vz_err = des_vz - v[2]
        des_az = self.Kd_alt * vz_err

        # Thrust: counteract gravity + provide vertical acceleration
        # In NED: motors push in -Z body, gravity is +9.81 in world Z
        thrust_z = self.mass * (9.81 - des_az)
        thrust_mag = np.clip(thrust_z, 0.0, 20.0)

        # Attitude stabilization: hold level (roll=0, pitch=0, yaw=0)
        roll_err = -euler.roll
        pitch_err = -euler.pitch
        yaw_err = -euler.yaw

        tau_x = self.Kp_att * roll_err - self.Kd_att * omega[0]
        tau_y = self.Kp_att * pitch_err - self.Kd_att * omega[1]
        tau_z = self.Kp_att * 0.3 * yaw_err - self.Kd_att * omega[2]

        tau = np.clip(np.array([tau_x, tau_y, tau_z]), -0.5, 0.5)

        thrust = BodyFixed(0.0, 0.0, float(thrust_mag))
        torque = BodyFixed(float(tau[0]), float(tau[1]), float(tau[2]))
...
class DefaultIntegrator(IntegratorBase):
    ...
    def integrate(
        ...
        acc_total = BodyFixed.from_Array(
            acc.vec + gravity_body, flag="acceleration"
        )
